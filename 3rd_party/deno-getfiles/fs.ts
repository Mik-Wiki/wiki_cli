...
export async function findFile({ path, collect, exclude, ignore, hasInfo, isFirst }: FindFileOptions): Promise<void> {
  ...
  try {
    ...
    throw new Error("NotFound: No such file or directory"); // why the fuck was here a Deno.exit(-1)???????????????
  }
  ...
  for (const item of rs) {
    ...
    if (item.isDirectory) {
	  ...
	  // check if _path exsits
	  try {
		if (!Deno.statSync(_path).isFile) {
			//debug_log(`Weird... ${_path} is not a file\n`);
			continue;
		}
	  } catch (e) {
		//debug_log(`Weird... ${_path} is not a file\n`);
